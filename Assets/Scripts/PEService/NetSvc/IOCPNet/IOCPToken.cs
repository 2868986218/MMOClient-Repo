...
namespace SUNet
{
    ...
    public abstract class IOCPToken<T> where T : new() {
        ...
        public bool SendMsg(byte[] bytes) {
            if (tokenState != TokenState.Connected) {
                IOCPTool.Warn($"TokenID:{tokenID} Connection is Disconnected,cannot send netSvc msg.");
                return false;
            }
        ...
        void IO_Completed(object sender, SocketAsyncEventArgs saea) {
            switch (saea.LastOperation) {
                ...
                case SocketAsyncOperation.Send:
                    ...
                    break;
                default:
                    IOCPTool.Warn($"TokenID:{tokenID} The last operation completed on the socket was not a receive or send op.");
                    break;
            }
        }
